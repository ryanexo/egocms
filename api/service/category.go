...
import (
    ...
    `dpcms/api/errors/category_error`
    `dpcms/api/infra`
    `dpcms/model`
    `dpcms/model/query`
...
//
// ParentID为0时，视为根节点
func (srv *CategoryService) Create(ctx context.Context, category *model.Category) error {
    return srv.query.Transaction(func(tx *query.Query) error {
        queryCtx := tx.WithContext(ctx)
        err := queryCtx.Category.Create(category)
        if err != nil {
            return err
        }
...
func (srv *CategoryService) Move(ctx context.Context, id int64, parent int64) error {
    return srv.query.Transaction(func(tx *query.Query) error {
        ...
        if err == nil {
            ...
            return category_error.ErrCircularReferenceWhenMove.Format(targetCategory.Name, currentCategory.Name).ToError()
...
func (srv *CategoryService) FindDescendant(ctx context.Context, id int64, distance int64) ([]*model.Category, error) {
    queryCtx := srv.query.WithContext(ctx)
    categoryCtxDAO := srv.query.CategoryContext
    cond := categoryCtxDAO.Distance.Lt(distance)
    if distance == 0 {
        cond = categoryCtxDAO.Distance.Gt(0)
    }
    descendants := queryCtx.CategoryContext.Select(categoryCtxDAO.ID).Where(cond).Where(categoryCtxDAO.Ancestor.Eq(id))
    return queryCtx.Category.Where(gen.Exists(descendants)).Find()
}

func (srv *CategoryService) ListRootNodes(ctx context.Context, pageNo int, pageSize int) ([]*model.Category, error) {
    return srv.query.Category.WithContext(ctx).Scopes(db_scopes.Paginate(pageNo, pageSize)).Where(srv.query.Category.ParentID.Eq(0)).Find()
}

func (srv *CategoryService) ListNodesByParentID(ctx context.Context, id int64, pageSize int, pageNo int) ([]*model.Category, error) {
    q := srv.query.Category
    return q.WithContext(ctx).Where(q.ParentID.Eq(id)).Scopes(db_scopes.Paginate(pageNo, pageSize)).Find()
}
