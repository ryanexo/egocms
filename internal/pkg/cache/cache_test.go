...
var (
    instance = New(WithDefaultTTL(time.Millisecond*100), WithClearInterval(time.Millisecond*300), WithSegmentCount(4))
)
...
func TestContainer_Flush_Delete(t *testing.T) {
    ...
    instance.Flush()
    n := 0
    for i := 0; i < instance.sgCount; i++ {
        n += len(instance.segments[i].items)
    }
    assert.Equal(t, 0, n)
...
func Test_Inc_Dec(t *testing.T) {
    instance.SetWithTTL("k", uint8(15), time.Duration(time.Hour))
