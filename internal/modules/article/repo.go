...
func NewArticleRepo(persist *gquery.Query) contract.ArticleRepo {
    return articleRepo{persist}
}

func (s articleRepo) CloneWithQuery(q *gquery.Query) contract.ArticleRepo {
...
func (s articleRepo) ReplaceKeywords(ctx context.Context, id datatype.SafeUint64, keywords []string) error {
    ...
    if len(keywords) > 0 {
        keywordSlice := make([]*model.ArticleTag, 0, len(keywords))
        for i := range keywords {
            keywordSlice[i] = &model.ArticleTag{ArticleID: id, Keyword: keywords[i]}
        }
