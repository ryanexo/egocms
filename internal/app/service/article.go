...
func (s Article) ChangeStatus(ctx context.Context, id datatype.SafeUint64, action func(status *artDomain.Status) error) error {
    artRepo := repo.NewArticle(s.persist)
    data, err := artRepo.FindByIDWithoutPreload(ctx, id)
    if err != nil {
        return err
    }
    status := artDomain.NewStatus(data.Status)
    if err = action(status); err != nil {
        return err
    }
    _, err = repo.NewArticle(s.persist).UpdateStatus(ctx, id, status.Value())
    return err
}
