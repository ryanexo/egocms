...
//
// ParentID为0时，视为根节点
func (srv MenuService) Create(ctx context.Context, params menu.CreateParams) (result menu.DetailResult, err error) {
...
func (srv MenuService) Update(ctx context.Context, params menu.UpdateParams) (result menu.DetailResult, err error) {
...
func (srv MenuService) Delete(ctx context.Context, id int64) error {
    return srv.query.Transaction(func(tx *query.Query) error {
        ...
        descendants, err := queryCtx.Menu.Where(menuDAO.ParentID.Eq(id)).Find()
        if err != nil {
            return err
        }
...
func (srv MenuService) Move(ctx context.Context, id int64, parent int64) error {
    return srv.query.Transaction(func(tx *query.Query) error {
        ...
        if err == nil {
            ...
            return erroz.ErrMenuCircularReferenceWhenMove.Format(targetCategory.Name, currentCategory.Name).ToError()
...
func (srv MenuService) FindByID(ctx context.Context, id int64) (menu.DetailResult, error) {
    q := srv.query.Menu
    item, err := q.WithContext(ctx).Where(q.ID.Eq(id)).First()
    if err != nil {
        return menu.DetailResult{}, err
    }
    m := menu.DetailResult{}
    err = copier.Copy(&m, item)
    return m, nil
}

func (srv MenuService) List(ctx context.Context, condition *menu.ListParams) (result types.PaginatedResult[menu.DetailResult], err error) {
    menuDAO := srv.query.Menu
    ...
    details := make([]menu.DetailResult, len(menuList), len(menuList))
    err = copier.Copy(&details, menuList)
    if err != nil {
        return
    }
    result = types.PaginatedResult[menu.DetailResult]{
        Total:    count,
        List:     details,
