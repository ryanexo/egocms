...
import (
    ...
    contract2 "cms/internal/app/contenttype/contract"
    "cms/internal/app/contenttype/internal/assembler"
...
func NewArticleModelService(txManager persistence.Transactor, repo contract2.ArticleModelRepo) *ArticleModelService {
...
func (s ArticleModelService) ReplaceSchema(ctx context.Context, params dto.ArticleModelSchemaUpdateParams) error {
    ...
    schema := make([]*model.ContentTypeSchema, 0, len(params.Data))
    
    for _, item := range params.Data {
        tmpSchema := assembler.ToArticleModelSchemaModel(item)
        if item.ID != nil {
            tmpSchema.ContentTypeID = params.ID
        }
