...
func (s Service) ChangeStatus(ctx context.Context, id datatype.SafeUint64, action func(status *domain.Status) error) error {
...
func (s Service) ToArticleModelData(artID datatype.SafeUint64, modelID datatype.SafeUint64, allSchema []*model.ArticleModelSchema, data map[string]any) (*model.ArticleModelJsonData, []*model.ArticleModelData, error) {
	...
	for _, schema := range allSchema {
		...
		artModel := domain.NewModelData(assembler.NewRules(schema), v)
