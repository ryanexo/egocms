...
import (
    ...
    `dpcms/internal/http/errors/category_error`
...
func (srv *CategoryService) ListRootNodes(ctx context.Context, pageNo int, pageSize int) ([]*model.Category, error) {
    return srv.query.Category.WithContext(ctx).Scopes(dbscopes.Paginate(pageNo, pageSize)).Where(srv.query.Category.ParentID.Eq(0)).Find()
}
