...
func (s MenuRepo) Create(ctx context.Context, menu *model.Menu) error {
    return s.persist.Menu.WithContext(ctx).Create(menu)
}

func (s MenuRepo) CreateSubtree(ctx context.Context, id uint64, parentId uint64) error {
    return s.persist.MenuContext.WithContext(ctx).CreateSubtree(id, parentId)
}

func (s MenuRepo) Update(ctx context.Context, menu *model.Menu) (gen.ResultInfo, error) {
    return s.persist.Menu.WithContext(ctx).Updates(menu)
}

func (s MenuRepo) Move(ctx context.Context, fromNode uint64, toNode uint64) error {
    ctxDao := s.persist.MenuContext
    catDao := s.persist.Menu
...
func (s MenuRepo) Delete(ctx context.Context, id uint64) error {
    ...
    err := s.persist.Menu.WithContext(ctx).UnderlyingDB().Exec(deleteCategorySql, id).Error
    if err != nil {
        return err
    }
    
    err = s.persist.MenuContext.WithContext(ctx).DropSubtree(id)
    if err != nil {
        return err
    }
...
func (s MenuRepo) FindByID(ctx context.Context, id uint64) (*model.Menu, error) {
    return s.persist.Menu.WithContext(ctx).Where(s.persist.Menu.ID.Eq(id)).First()
}
