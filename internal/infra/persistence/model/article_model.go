...
type ArticleModelJsonData struct {
    ...
    Data      string `gorm:"type:text;comment:'文章的模型json数据'"`
}
...
type ArticleModelSchema struct {
    ...
    MinLen      uint64              `gorm:"default:0;comment:'最小长度'"`
    MaxLen      uint64              `gorm:"default:0;comment:'最大长度'"`
    ...
    Sequence    int64               `gorm:"index;default:0;comment:'字段排序'"`
    Type        int16               `gorm:"type:smallint;not null;comment:'字段值类型'"`
    EnumOptions string              `gorm:"type:text;comment:'字段枚举json'"`
    Required    int8                `gorm:"type:tinyint;default:0;comment:'是否必填'"`
    Hidden      int8                `gorm:"type:tinyint;default:0;comment:'是否隐藏'"`
    Enable      int8                `gorm:"type:tinyint;default:1;comment:'是否启用'"`
}
