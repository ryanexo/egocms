...
func newCategory(db *gorm.DB, opts ...gen.DOOption) category {
	...
	_category.DeletedAt = field.NewTime(tableName, "deleted_at")
	...
	_category.Meta = categoryHasOneMeta{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Meta", "model.CategoryMeta"),
	}
...
func (c category) clone(db *gorm.DB) category {
	...
	c.Meta.db = db.Session(&gorm.Session{Initialized: true})
	c.Meta.db.Statement.ConnPool = db.Statement.ConnPool
	return c
}

func (c category) replaceDB(db *gorm.DB) category {
	c.categoryDo.ReplaceDB(db)
	c.Meta.db = db.Session(&gorm.Session{})
	return c
}

type categoryHasOneMeta struct {
	db *gorm.DB

	field.RelationField
}

func (a categoryHasOneMeta) Where(conds ...field.Expr) *categoryHasOneMeta {
	if len(conds) == 0 {
		return &a
	}
...
func (a categoryHasOneMeta) WithContext(ctx context.Context) *categoryHasOneMeta {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a categoryHasOneMeta) Session(session *gorm.Session) *categoryHasOneMeta {
	a.db = a.db.Session(session)
	return &a
}

func (a categoryHasOneMeta) Model(m *model.Category) *categoryHasOneMetaTx {
	return &categoryHasOneMetaTx{a.db.Model(m).Association(a.Name())}
}

func (a categoryHasOneMeta) Unscoped() *categoryHasOneMeta {
	a.db = a.db.Unscoped()
	return &a
}

type categoryHasOneMetaTx struct{ tx *gorm.Association }

func (a categoryHasOneMetaTx) Find() (result *model.CategoryMeta, err error) {
	return result, a.tx.Find(&result)
}

func (a categoryHasOneMetaTx) Append(values ...*model.CategoryMeta) (err error) {
...
func (a categoryHasOneMetaTx) Replace(values ...*model.CategoryMeta) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a categoryHasOneMetaTx) Delete(values ...*model.CategoryMeta) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a categoryHasOneMetaTx) Clear() error {
	return a.tx.Clear()
}

func (a categoryHasOneMetaTx) Count() int64 {
	return a.tx.Count()
}

func (a categoryHasOneMetaTx) Unscoped() *categoryHasOneMetaTx {
	a.tx = a.tx.Unscoped()
	return &a
}
