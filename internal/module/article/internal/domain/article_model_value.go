...
type ArticleModelValue struct {
    value Value
    rules Rules
}

func NewModelValue(rules Rules, value Value) *ArticleModelValue {
    return &ArticleModelValue{rules: rules, value: value}
}

func (v *ArticleModelValue) IsValid() error {
    if !v.rules.IsEnable() {
...
func (v *ArticleModelValue) Assign(s Scannable) error {
