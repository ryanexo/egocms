...
func NewArticleModelRepo(persist *query.Query) article.ArticleModelRepo {
    return &articleModelRepo{persist}
}

func (r *articleModelRepo) CloneWithQuery(q *query.Query) article.ArticleModelRepo {
...
func (r *articleModelRepo) List(ctx context.Context, params dto.ArticleModelListParams) ([]*model.ArticleModel, int64, error) {
    m := r.query.ArticleModel
    q := m.WithContext(ctx).Scopes(scope.Paginate(params.PageNo, params.PageSize))
    if params.Name != nil {
